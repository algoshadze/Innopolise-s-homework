...
module.exports = async () => {
    ...
    fs.exists(entryPath, (exists) => {
        if (!exists) {
            ...
            fs.writeFile(entryPath, createEntry(), () => logSuccess('src/index.tsx создан.'));
    ...
    fs.exists(appPath, (exists) => {
        if (!exists) {
            const createEntry = require('./templates/app');

            fs.writeFile(appPath, createEntry(package.name), () => logSuccess('src/app.tsx создан.'));

        } else {
            logWarning('src/app.tsx Уже существует, пропускаю этот шаг.');
        }
    })
